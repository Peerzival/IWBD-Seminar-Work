...
ggplot(plot_data, aes(x = Measure, y = Score, fill = Group)) +
...
### Analyzing Satisfaction Over Time
...
PITU_SAT <- data_only_free_activity %>%
  filter(ID == id) %>%
  select(DATE, EV_PITU_Satisfaction)  %>%
  mutate(EV_PITU_Satisfaction = as.numeric(EV_PITU_Satisfaction))

SP_SAT <- data_only_free_activity %>%
  filter(ID == id) %>%
  select(EV_SP_Satisfaction)  %>%
  mutate(EV_SP_Satisfaction = as.numeric(EV_SP_Satisfaction))

SM_SAT <- data_only_free_activity %>%
  filter(ID == id) %>%
  select(EV_SM_Satisfaction)  %>%
  mutate(EV_SM_Satisfaction = as.numeric(EV_SM_Satisfaction))

combined_data <- cbind(PITU_SAT, SP_SAT, SM_SAT)

combined_data_long <- combined_data %>%
  pivot_longer(cols = c(EV_PITU_Satisfaction, EV_SP_Satisfaction, EV_SM_Satisfaction),
               names_to = "Satisfaction_Type",
               values_to = "Satisfaction_Score")
```

**Plotting satisfaction over weak**

```{r}
my_colors <- brewer.pal(3, "Dark2")
ggplot(combined_data_long, aes(x = DATE, y = Satisfaction_Score, color = Satisfaction_Type)) +
  geom_point() +
  geom_line() +
  theme_minimal() +
  theme(axis.title.x = element_text(size = 9, face = "bold"),
        ...
        legend.text = element_text(size = 7),
        legend.key.size = unit(0.3,"cm"),
        legend.position.inside = c(0.99, 0.2),
        legend.position = "inside",
        legend.justification='right',
        legend.box.background = element_rect(color = "grey95", fill = "grey95", size = 0.1),
    legend.background = element_rect(fill = "transparent", linewidth = 0.)) +
  labs(x = "Day",
       y = "Satisfaction Score",
       color = "Satisfaction Type") +
  scale_color_manual(values = my_colors,
                    guide = guide_legend(nrow = 3, ncol = 1, byrow = TRUE, title.position = "top", title.hjust = 0)) +
...
```{r}
ggcorrplot(correlation_matrix_EX, 
           method ="square", 
           hc.order = TRUE,
           lab = TRUE,
           outline.color = "white", 
...
my_plot <- ggplot(combined_data_long, aes(x = value)) +
  geom_bar(aes(y = n, fill = Activity, alpha = Activity), stat = "identity", position = "identity", width = 0.5, alpha = 0.6) +
...
#ggsave("/Users/max/Desktop/my_plot.pdf", my_plot, width = 10, height = 8)
print(my_plot)
```
...
SP_EX_avg <- data_only_free_activity %>% 
  ...
  group_by(DATE) %>% 
  mutate(EV_SP_Experiences = as.numeric(EV_SP_Experiences)) %>%
  summarise(EV_SP_Experiences = mean(EV_SP_Experiences, na.rm = TRUE)) %>%
  ungroup()
